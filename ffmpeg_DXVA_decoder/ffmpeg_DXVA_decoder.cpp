...
static IDXGISwapChain2 *pSwapChain2 = NULL;
...
static HRESULT d3d11_init(HWND hWnd)
{
	...
	hr = pIDXGIFactory3->CreateSwapChainForHwnd(m_pD3D11Device, hWnd, &swapChainDesc, NULL, NULL, &pSwapChain1);
	if (FAILED(hr)) goto done;

	pSwapChain2 = (IDXGISwapChain2 *)pSwapChain1;
	...
	hr = m_pD3D11Device->CreateRenderTargetView(pBackBuffer, nullptr, &pRenderTargetView);
	if (FAILED(hr)) goto done;

	// Set new render target
	m_pD3D11DeviceContext->OMSetRenderTargets(1, &pRenderTargetView, nullptr);
...
HRESULT SetSuperResNvidia(const bool enable)
{
	...
	HRESULT hr = m_pD3D11VideoContext->VideoProcessorSetStreamExtension(
		m_pD3D11VideoProcessor, 0, &kNvidiaPPEInterfaceGUID,
...
HRESULT SetSuperResIntel(const bool enable)
{
	...
	HRESULT hr = m_pD3D11VideoContext->VideoProcessorSetOutputExtension(m_pD3D11VideoProcessor, &GUID_INTEL_VPE_INTERFACE, sizeof(ext), &ext);
	if (FAILED(hr)) {
		return hr;
	}
	...
	hr = m_pD3D11VideoContext->VideoProcessorSetOutputExtension(m_pD3D11VideoProcessor, &GUID_INTEL_VPE_INTERFACE, sizeof(ext), &ext);
	if (FAILED(hr)) {
		return hr;
	}
	...
	hr = m_pD3D11VideoContext->VideoProcessorSetStreamExtension(m_pD3D11VideoProcessor, 0, &GUID_INTEL_VPE_INTERFACE, sizeof(ext), &ext);
...
void d3d11va_retrieve_data(AVCodecContext *avctx, AVFrame *frame)
{
	...
	hr = pSwapChain2->GetBuffer(0, __uuidof(ID3D11Texture2D), (void**)&pDXGIBackBuffer);
	if (FAILED(hr)) return;
...
#endif
			// Create a render target view
			hr = m_pD3D11Device->CreateRenderTargetView(pDXGIBackBuffer, nullptr, &pRenderTargetView);
			if (FAILED(hr)) goto done;
...
DWORD WINAPI ThreadProc(_In_ LPVOID lpParameter)
{
	...
	const char *filename = "D:\\video\\movie\\360ptest.mp4";
